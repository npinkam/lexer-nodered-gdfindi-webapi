...
<script>
    ...
    node.on("input", function (msg, done) {
      ...
      xhr.open(
        "GET",
        utility.gdFindiUrl() + `/api/v1/PVDO/${MiningID}/Results`,
        false
      );
      ...
      if (response.results === undefined || response.results.length == 0) {
        ...
        msg.payload = "";
        msg.payload = utility.htmlTemplate(
          title,
          library,
          style,
          header,
        ...
        for (var i = 0; i < result.length; i += 2) {
          ...
          //remove first str
          timeline.shift();
          cumTimeline = timeline.map(
            (elem) => (sum = (sum || 0) + parseInt(elem))
          );
...
`;
  ...
  RED.nodes.registerType(
    "PVDO: Workload Timeline",
    gdfindiWebapiPVDOWorkloadTimelineNode
  );
};
